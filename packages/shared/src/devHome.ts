 ...
 * `~/.harness` that a user's installed Harness runs against.
 *
 * A linked git worktree gets its own (gitignored) `.harness`: feature work in a
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".harness");
  });
