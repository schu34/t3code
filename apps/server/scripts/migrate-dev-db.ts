 ...
 * ~/.harness database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.harness/userdata/state.sqlite`.
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedError<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .harness directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedError<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.harness database. Use an isolated --base-dir.";
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .harness directory. Defaults to `<worktree>/.harness` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.harness/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".harness"));
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.String("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .harness directory. Defaults to the current worktree's .harness.",
      ),
    ),
    source: Flag.String("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.harness/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.harness data, then run migrations.",
  ),
);
